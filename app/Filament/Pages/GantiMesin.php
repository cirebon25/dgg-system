...
class GantiMesin extends Page implements HasForms
{
    ...
    public function loadRiwayat(): void
    {
        ...
        $this->riwayat = MachineReplacement::with([
            'customer',
            'oldMachine',
            'newMachine',
            'technician',
            ...
            ->map(fn($r) => [
                'id'                  => $r->id,
                'tanggal'             => $r->tanggal,
                'nama_customer'       => $r->customer?->nama_customer,
                'sn_lama'             => $r->oldMachine?->serial_number,
                'sn_baru'             => $r->newMachine?->serial_number,
                'counter_bw_final'    => $r->counter_bw_final,
                'counter_color_final' => $r->counter_color_final,
                'nama_technician'     => $r->technician?->nama_technician,
    ...
    public function submit(): void
    {
        ...
        DB::transaction(function () use ($data, &$newDeploymentId, &$replacementId) {
            ...
            // 2. ✅ MESIN LAMA → Ready & SIMPAN COUNTER AKHIRNYA
            $oldMachine->update([
                'status'           => 'Ready',
                'customer_id'      => null,
                'counter_bw'       => $data['counter_bw_final'],      // ✅ SIMPAN COUNTER AKHIR
                'counter_color'    => $data['counter_color_final'],   // ✅ SIMPAN COUNTER AKHIR
                'last_rolled_at'   => $data['tanggal'],               // ✅ SIMPAN TANGGAL ROLLING
            ]);

            // 3. MESIN BARU → Rented & SET COUNTER AWAL
            $newMachine->update([
                'status'           => 'Rented',
                'customer_id'      => $customer->id,
                'counter_bw'       => $data['counter_bw_awal'] ?? 0,     // ✅ SIMPAN COUNTER AWAL
                'counter_color'    => $data['counter_color_awal'] ?? 0,  // ✅ SIMPAN COUNTER AWAL
            ...
            // 5. Simpan deployment_id ke machine_replacements
            DB::table('machine_replacements')
                ->where('id', $replacementId)
                ->update(['deployment_id' => $newDeploymentId]);

            // 6. Potong stok GUDANG & catat ke deployment_sparepart
            foreach (($data['spareparts'] ?? []) as $part) {
                if (empty($part['sparepart_id'])) continue;
        ...
        // 7. Ambil data ringkas untuk notifikasi
        $dep = DB::table('deployments')
            ->where('deployments.id', $newDeploymentId)
            ->join('customers', 'deployments.customer_id', '=', 'customers.id')
            ->join('machines', 'deployments.machine_id', '=', 'machines.id')
        ...
        // 8. Notifikasi sukses + tombol cetak SJ
        Notification::make()
            ->title('✅ Rolling Berhasil!')
            ->body("Mesin {$rep?->old_sn} → {$dep?->new_sn} untuk {$dep?->nama_customer}")
            ->success()
