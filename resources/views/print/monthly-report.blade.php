...
<body onload="window.print()">
    <div class="no-print">
        <button onclick="window.print()"
            style="padding: 10px 20px; background: #22c55e; color: white; border: none; cursor: pointer; border-radius: 5px;">
            🖨️ CETAK SEKARANG
        </button>
        <button onclick="window.history.back()"
            style="padding: 10px 20px; background: #6b7280; color: white; border: none; cursor: pointer; border-radius: 5px; margin-left: 10px;">
            ⬅️ KEMBALI
        </button>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="4%">Tipe</th>
                <th width="15%">Perbaikan</th>
                <th width="12%">Sparepart</th>
        ...
        <tbody>
            @foreach ($logs as $log)
                <tr>
                    ...
                    <td><strong>{{ $log->machine->deployment->no_kontrak ?? '-' }}</strong></td>

                    <td>{{ \Carbon\Carbon::parse($log->tanggal)->format('d/m/Y') }}</td>

                    <td>
                        <strong>{{ $log->machine->deployment->customer->nama_customer ?? '-' }}</strong><br>
                        Model: {{ $log->machine->tipe_model ?? '-' }}<br>
                        Instal:
                        {{ $log->machine->deployment?->tanggal_instal ? \Carbon\Carbon::parse($log->machine->deployment->tanggal_instal)->format('d/m/Y') : '-' }}
                    </td>
                        ...
                        @if ($log->serviceLogSpareparts && $log->serviceLogSpareparts->count() > 0)
                            <ul style="margin: 0; padding-left: 15px;">
                                @foreach ($log->serviceLogSpareparts as $sp)
                                    <li>{{ $sp->sparepart->nama_sparepart ?? 'Tidak Diketahui' }}
                                        ({{ $sp->jumlah }})</li>
                                @endforeach
                            </ul>
                        @else
                    ...
                    <td>
                        ...
                        {{-- Menggunakan !empty agar string kosong tetap terdeteksi dan tercetak --}}
                        @if (!empty($log->nama_teknisi_manual))
                            <br><small style="color: #374151; font-weight: bold;">Partner:
                                {{ $log->nama_teknisi_manual }}</small>
                        @endif
                    </td>
                </tr>
            @endforeach
